...
if __name__ == '__main__':
    ...
    fold = 0
    alpha = 0.01
    logging.basicConfig(filename='logs{:d}/train_resnet.{:d}.log.vad{:s}'.format(sampleSize, fold, str(alpha)),
                        level=logging.INFO)
    rePrint('-----------------------------------------------------------')
    sr = 22050
    steps = 1
    ...
    modelName = 'resnet{:d}.{:d}.model.vad{:s}'
    ...
    for epoch in range(epochs):
        ...
        modelFile = os.path.join(modelPath, modelName.format(sampleSize, epoch, str(alpha)))
