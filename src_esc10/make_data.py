...
def get_samplelist(audioName, audioData):
    ...
    sampleList = []
    for sPoint, ePoint in silentDictList[audioName]:
        #print sPoint, ePoint
...
if __name__ == '__main__':
    ...
    silentDictListFile = 'vad-dict/vad{:s}.dictlistsmooth'.format(str(alpha))

    feaPath = '/home/pdl/Desktop/media/feature/feature_ESC-10/22050/logfbank'
    dataPath = '/home/pdl/Desktop/media/data/data_ESC-10/{:d}_{:d}'.format(sr, fold)

    check_file(trainFile)
    check_file(validFile)
    check_file(testFile)
    check_file(silentDictListFile)

    check_path(feaPath)
    make_path(dataPath)

    silentDictList = pickle.load(open(silentDictListFile, 'rb'))
