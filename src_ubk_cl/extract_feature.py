...
def extract_feature(y, sr):
    ...
    #   signal, sr=22050, n_fft=1024, hop_length=512, n_mels=BANDS)
    ...
    #print 'log_spect: ', type(log_spect), log_spect.shape
    return log_spect
...
if __name__ == '__main__':
    ...
    audioPath = '/home/lj/work/project/dataset/data_UrbanSound8K/audio'
    check_path(audioPath)

    featPath = '../../feature/feature_ubk/{:d}_logspec'.format(sr)
    make_path(featPath)
    ...
    for idx, items in enumerate(lines):
        ...
        pickle.dump(data, open(os.path.join(featPath, audioName.replace('.wav', '.fea')), 'wb'),
